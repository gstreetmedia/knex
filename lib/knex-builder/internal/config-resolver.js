...
function resolveConfig(config) {
  ...
  // If neither applies, let's assume user specified name of a client or dialect as a string
    ...
    // If the connection config contains a url and the client is sqlite or better-sqlite, use libsql (aka turso)
    if ((clientName === "sqlite3" || clientName === "better-sqlite3") &&
      typeof parsedConfig.connection === "object" && "url" in parsedConfig.connection) {
      Dialect = getDialectByNameOrAlias("libsql");
    } else {
